 ...
 *  showing focus intensity over the past 12 weeks.
 * ───────────────────────────────────────────────────── */
...
function getColor(minutes: number): string {
  ...
  if (minutes < 15) return 'bg-tomato-900/60';
  if (minutes < 30) return 'bg-tomato-800/70';
  if (minutes < 60) return 'bg-tomato-700/80';
  if (minutes < 90) return 'bg-tomato-600/90';
...
export function Heatmap({ dailyStats }: HeatmapProps) {
  const { grid, monthLabels } = useMemo(() => {
    ...
    startDate.setDate(
      today.getDate() - (WEEKS - 1) * 7 - dayOfWeek,
    );
    ...
    <div className="glass rounded-xl p-3">
      ...
      <div className="flex items-center gap-1 mt-2 justify-end">
        ...
        {[0, 15, 30, 60, 90].map((m) => (
          <div
            key={m}
            className={`w-2.5 h-2.5 rounded-sm ${getColor(m)}`}
          />
        ))}
